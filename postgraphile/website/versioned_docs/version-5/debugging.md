...
- Functions that use `returns table(...)` cannot have this return type augmented
  with comments and smart tags (nor can you comment on/tweak the attributes); the
  return type will be exposed as a GraphQL type with an autogenerated name.
  Avoid this construct; instead, you should always prefer explicit named types
  (`returns setof my_type`) - see [Adding a field to a function-derived
  type][add-field-function-derived-type]
- Computed columns must follow the naming and signature rules in [Computed
  columns](./computed-columns) (remember `STABLE`!), and must be in the same
  PostgreSQL schema as the table they apply to
- Views do not have foreign key constraints, so relations are not inferred;
  add `@foreignKey` smart tags to the view as described in
  [Views](./views.md) and [Relations](./relations.md). If referencing a view,
  ensure the view has an `@primaryKey`
...
[add-field-function-derived-type]: ./customization-overview.md#adding-a-field-to-a-function-derived-type
