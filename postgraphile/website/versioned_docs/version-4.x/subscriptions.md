...
The endpoint for subscriptions is the same as for GraphQL, except the
...
:::note
This section only applies to "simple subscriptions."
:::
...
\* The prefix is applied by default, but you can override it via
...
This solution is still taking shape, so it's not yet certain how
...
In PostgreSQL the channel is an "identifier" which by default is
