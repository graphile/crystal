...
relevant details to PostgreSQL via [`pgSettings`](./config.mdx#pgsettings) so that
Row Level Security (RLS) policies and functions can make decisions. See the
...
If you are using Apollo’s websocket transport:
