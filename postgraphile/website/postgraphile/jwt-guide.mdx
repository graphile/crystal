...
identify who is making the request. The key requirement is that you surface the
relevant details to PostgreSQL via [`pgSettings`](./config#pgsettings) so that
Row Level Security (RLS) policies can make decisions. See the
[security](./security) page for the broader architecture and trade-offs.

This guide explains how JWTs fit into that picture, how to populate `pgSettings`
from a verified token, and when you might lean on the
`postgraphile/presets/lazy-jwt` convenience preset.

:::info[You own JWT verification]

PostGraphile V5 no longer auto-verifies JWTs. You are expected to verify tokens
in your server framework (Express, Koa, Fastify, etc.) and feed the resulting
claims into PostgreSQL yourself. This means you can implement refresh tokens,
revocation, clock skew handling, and other behaviours that PostGraphile cannot
reasonably guess for you.
...
- Anything placed inside `pgSettings` becomes available via
  `current_setting('jwt.claims.foo', true)` when RLS policies run.
- Do not leak entire JWT payloads blindly—only forward what your schema needs.

If you want to understand how claims map onto PostgreSQL session variables, see
the [JWT specification](./jwt-specification) for the precise rules.
...
- runs only when you use a `grafserv`-compatible HTTP adaptor;
- reads the `Authorization` header for Bearer tokens;
- verifies the token with `jsonwebtoken` using a shared secret (default
  algorithms `HS256`/`HS384`, default audience `postgraphile`, override via
  `preset.grafserv.pgJwtSecret` and
...
If you still rely on JWTs but want to generate them from your schema,
PostGraphile can sign composite types listed in `preset.gather.pgJwtTypes`.
When a function
returns one of those types the response will contain a signed JWT string.
...
appropriately. The
[`pgSettings` section of the config docs](./config#pgsettings)
shows how to expose request data regardless of authentication style, and the
[security](./security) page lists common approaches. Pick the strategy that
fits your infrastructure, and use JWTs only if they solve a problem you
actually have.
