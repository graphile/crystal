...
[`extendSchema`](./extend-schema) to extend the
...
the payload `{"event": "create", "sub": FORUM_ID, "id": MESSAGE_ID}`. This
event will be picked up by the `pgSubscriber`, which `listen()` subscribes to.
This event will then be parsed by `listen()` using the `itemPlan` option (set
to `jsonParse`), and the resulting object will be passed to the
`Subscription.forumMessage.plan()` plan resolver, which does no further
processing. The data then flows down to the
...
const MySubscriptionPlugin = extendSchema((build) => {
  const { messages } = build.pgResources;
  return {
    typeDefs: /* GraphQL */ `
