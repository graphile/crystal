...
Many users had package-mismatches and duplicate package issues with PostGraphile
V4, so with V5 most of the core packages are available via
[`exports`](https://nodejs.org/api/packages.html#exports) - so you can typically
get away with just installing `postgraphile` and any plugins you need.

| Instead of `import ...`    | Use `import ...`                       |
| -------------------------- | -------------------------------------- |
| `from 'graphile-build'`    | `from 'postgraphile/graphile-build'    |
| `from 'graphile-build-pg'` | `from 'postgraphile/graphile-build-pg' |
| `from 'graphql'`           | `from 'postgraphile/graphql'           |
| `from 'pg-sql2'`           | `from 'postgraphile/pg-sql2'           |
| `from 'graphile-utils'`    | `from 'postgraphile/utils'             |

The same is true for builtins in the new system as well:

| Instead of `import ...`           | Use `import ...`                         |
| --------------------------------- | ---------------------------------------- |
| `from 'grafast'`                  | `from 'postgraphile/grafast'             |
| `from 'grafserv'`                 | `from 'postgraphile/grafserv'            |
| `from 'grafserv/express/v4'`      | `from 'postgraphile/grafserv/express/v4' |
| `from '@dataplan/json'`           | `from 'postgraphile/@dataplan/json'      |
| `from '@dataplan/pg'`             | `from 'postgraphile/@dataplan/pg'        |
| `from '@dataplan/pg/adaptors/pg'` | `from 'postgraphile/adaptors/pg'         |

So if your previous package.json looked like:
...
You can remove the extra packages and simply run:
...
Many of the V5 extension modules are now technically "presets" rather than
"plugins" - plugins go into the `plugins` list in your configuration, but
presets go into the `extends` list:
...
- `import { PostGraphileFulltextFilterPlugin } from '@graphile-contrib/postgraphile-plugin-fulltext-filter';`

Changed:

- ~~`@graphile/pg-pubsub`~~ is now built in; see [the subscriptions
  docs](../subscriptions.mdx) for more details. Users of the V4 preset can use
  `simpleSubscriptions: true` as before; those not using it can use the
  [PgV4SimpleSubscriptionsPlugin](https://github.com/graphile/crystal/blob/main/postgraphile/postgraphile/src/plugins/PgV4SimpleSubscriptionsPlugin.ts).
- ~~`@graphile-contrib/pg-simplify-inflector`~~ is renamed:  
...
Need more plugins ported? We need financial support! I'm sure there's a mutually
beneficial agreement to be found...
...
context](../config/reference#grafastcontext).
...
`addArgDataGenerator` and its ilk no longer exist, instead we declare
