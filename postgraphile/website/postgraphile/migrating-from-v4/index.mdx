...
V5 plugins use the `@rc` tag on npm during the release candidate phase. Also many are
now technically "presets" rather than "plugins" - plugins go into the `plugins`
list, but presets go into the `extends` list:
