...
- `--allow-explain` (alias: `-e`)
  <equivalent>{ `{ server: { exposePlan: true } }`}</equivalent>  
...
- `--connection <string>` (alias: `-c`)
  ...
  the PostgreSQL connection string. If omitted, inferred from environmental
  variables (see
...
- `--schema <string>` (alias: `-s`)
  <equivalent>{
  `{ pgSources: makePgSources(databaseUrl, ['...']) }`}</equivalent>  
