...
:::note["Smart tags" vs "smart comments"]

Smart tags are the annotations themselves; smart comments are one way to add
smart tags to an entity, specifically via PostgreSQL `COMMENT` statements.
...
## Adding smart tags to virtual constraints

If you need to apply any smart comments to a virtual constraint (sometimes
informally called a "fake constraint"), you cannot use newlines (`\n`) because
they will be interpreted as separate smart comments on the original entity.
We've added a workaround for this: you can use the pipe (`|`) symbol to assign
smart comments to the virtual constraint, for example to rename it:
