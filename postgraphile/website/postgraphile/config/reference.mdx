...
identifiers (for example if you want `select * from "users"` rather than `select
* from "public"."users"`); the runtime `search_path` then controls which schema
is targeted. This is sometimes used for multitenant databases where each tenant
has their own (identically structured) schema in the same databasee.
