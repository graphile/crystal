import { PgDeleteSingleStep, PgExecutor, PgResource, PgSelectStep, TYPES, assertPgClassSingleStep, makeRegistry, pgDeleteSingle, pgInsertSingle, pgSelectFromRecord, pgUpdateSingle, recordCodec, sqlFromArgDigests, sqlValueWithCodec } from "@dataplan/pg";
import { ConnectionStep, EdgeStep, ObjectStep, __ValueStep, access, assertEdgeCapableStep, assertExecutableStep, assertPageInfoCapableStep, bakedInput, bakedInputRuntime, connection, constant, context, createObjectAndApplyChildren, first, get as get2, inhibitOnNull, inspect, lambda, list, makeDecodeNodeId, makeGrafastSchema, object, rootValue, specFromNodeId, trap } from "grafast";
  ...
  Mutation: {
    ...
    getflamble: {
      plan($root, args, _info) {
        const selectArgs = makeArgs_renamed_function(args, ["input"]);
    ...
    filmEdge($mutation, fieldArgs) {
      ...
      const $select = (() => {
        if ($result instanceof PgDeleteSingleStep) {
          ...
          return resource_filmsPgResource.find(spec);
        }
      })();
    ...
    studioEdge($mutation, fieldArgs) {
      ...
      const $select = (() => {
        if ($result instanceof PgDeleteSingleStep) {
          ...
          return resource_studiosPgResource.find(spec);
        }
      })();
      fieldArgs.apply($select, "orderBy");
      const $connection = connection($select);
    ...
    tvEpisodeEdge($mutation, fieldArgs) {
      ...
      const $select = (() => {
        if ($result instanceof PgDeleteSingleStep) {
          ...
          return resource_tv_episodesPgResource.find(spec);
        }
      })();
      fieldArgs.apply($select, "orderBy");
      const $connection = connection($select);
    ...
    tvShowEdge($mutation, fieldArgs) {
      ...
      const $select = (() => {
        if ($result instanceof PgDeleteSingleStep) {
          ...
          return resource_tv_showsPgResource.find(spec);
        }
      })();
      fieldArgs.apply($select, "orderBy");
      const $connection = connection($select);
