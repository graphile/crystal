...
import { CollectionKey, BasicObjectType, BasicObjectField } from '../../../interface'
...
class PGCollectionKey implements CollectionKey<Value, Key> {
   ...
   */
  public type = this._pgKeyAttributes.reduce(
    (type, pgAttribute) => type.addField(new PGCollectionKeyBasicObjectField(this._pgCatalog, pgAttribute)),
    ...
    // make it public.
    new BasicObjectType(`_${this._pgConstraint.name}`)
  )
   ...
   */
  public getKeyFromValue (value: Value): Key {
    return this.type.getFields().reduce((key, field) => {
      key[field.getName()] = value[field.getName()]
      return key
    }, {})
  }
  ...
  public read = (
    !this._pgClass.isSelectable
      ? null
      : (context: mixed, key: Key): Promise<Value> => {
   ...
   */
  @memoizeMethod
  private _getSelectLoader (client: Client): DataLoader<Key, Value> {
    return new DataLoader<Key, Value>(
      async (keys: Array<Key>): Promise<Array<Value>> => {
        ...
        const query = sql.compile(sql.query`
          ${sql.join(keys.map(key =>
            sql.query`
              ...
              -- key value.
              where
                ${sql.join(this.type.getFields().map(field =>
                  sql.query`${sql.identifier((field as any).pgAttribute.name)} = ${sql.value(key[field.getName()])}`
                ), ' and ')}
            `
          ), ' union all ')}

          -- Lets set a limit to enforce we only get back the number of rows we
          -- expected.
...
class PGCollectionKeyBasicObjectField<TValue> extends BasicObjectField<TValue> {
  constructor (
    pgCatalog: PGCatalog,
    public pgAttribute: PGCatalogAttribute
  ) {
    super(
      pgAttribute.name,
      getTypeFromPGType(pgCatalog, pgCatalog.assertGetType(pgAttribute.typeId)),
    )
    this.setDescription(pgAttribute.description)
  }
}
