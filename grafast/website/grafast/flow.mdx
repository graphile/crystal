...
- [`inhibitOnNull`](./step-library/standard-steps/inhibitOnNull.mdx) skips
...
- [`trap`](./step-library/standard-steps/trap.mdx) recovers an inhibited or
  errored value and turns it back into ordinary data (for example a plain
  `null` or an empty list).

All three helpers wrap the `__FlagStep`. In plan diagrams the step is usually
absorbed into the dependency edge, so you will see labels such as `rejectNull`,
`trapError`, or `onReject="…"` rather than a dedicated node. Crucially,
Gra*fast* applies these flags per entry: if one item in a batch is inhibited or
errored it is simply omitted from the `execute()` call while the rest of the
items carry on unhindered.
