...
An execution value is a per-dependency input to a step's "execute" method,
carried via the execution details. It represents the execution result of one of
the step's dependencies.

Gra*fast* executes each step in a plan diagram just once for every request[^1],
even if that step is handling thousands of list values. For example, a step that
loads the author of a post will receive all author ids across all posts to which
it relates in one batched input (one "execution value"), rather than being
called once per author id.

For every dependency a step has, Gra*fast* takes the resulting "execution
value", automatically creates a derivative execution value if needed (stripping
out entries that should not be executed, e.g. due to error or inhibition, see
[flow control](#flow-control)), and passes it on to the step's `execute` method
for processing.

An "execution value" provides a common interface across both "unary" values
(for which there will always be a single value, independent of batch size) and
regular batched values. This allows steps to handle both types with common code
whilst maintaining execution efficiency. (Though optimizations are possible with
a little more code!)
...
step's execution. By default, any values flagged as error or inhibited will
also be filtered out, but this can be overridden; e.g. the
[`trap()`](./standard-steps/trap.mdx) step allows trapping errors or
inhibits.

:::note[Always skipped flags]

Any value flagged with "skipped due to polymorphism" or "unrecoverably stopped"
will be filtered out from the execution values passed to any future step
execution.
...
to represent this that will filter out these `null` values such that dependent
steps never need to process them.

### Abstract type boundaries

When an abstract type (interface or union) appears in a selection set, each
concrete type might require a different plan.

If this position is referenced across multiple existing polymorphic partitions,
first the relevant step(s) from these partitions are recombined via a "combined"
layer plan. This ensures that polymorphism doesn't cause the plan to
exponentially increase in size.

A layer plan boundary for that abstract position is created and used to plan
each possible type. Gra*fast* then groups together the types that end up with
the same plan to form polymorphic partitions. If there is only one partition, it
is eliminated for efficiency.

It's very common for abstract types to end up with the same plans - for example
`Cat`, `Dog`, and `Parrot` might all be sourced from the same "animals" data
store. Within a partition, certain steps may only be needed for certain types,
entries that do not match are automatically marked as skipped and filtered out
from downstream execution.

### Boundary summary

Layer plan boundaries are inserted whenever the batch size can change:

- list boundaries change the batch size to the number of list items, typically
  significantly increasing the batch size
- nullable boundaries remove `null` entries from downstream work, only ever
  decreasing the batch size
- polymorphic partition boundaries split the batch into different buckets for
  the remainder of execution
- combined layer plans are created to recombine multiple layer plans before
  proceeding into polymorphic planning
...
It is intended that step execute methods are generally lightweight if possible —
...
- [Flow control steps](./standard-steps/index.mdx#flow-control)
  documents the APIs (`inhibitOnNull`, `inhibitIf`, `assertNotNull`, `trap`).
...
When the early-exit condition is not simply "this value is null", use
`inhibitIf` instead. For example, if an empty list should skip downstream work,
derive a boolean condition step and inhibit on that:
...
- [`inhibitIf`](./standard-steps/inhibitIf.mdx) skips dependent work
  when a separate condition step yields `true`.
