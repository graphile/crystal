...
When an abstract type (interface or union) appears in a selection set, Gra*fast*
does not know at plan-time which concrete type each entry will be. It creates a
layer plan boundary for that abstract position and, at execution-time, splits
the batch into buckets by `__typename`. Each bucket then runs the steps for the
matching concrete type, while entries that do not match a branch are marked as
skipped and filtered out from downstream execution.

### Boundary summary

Layer plan boundaries are inserted whenever the batch size can change:

- list boundaries increase the batch size to the number of list items
- nullable boundaries remove `null` entries from downstream work
- abstract type boundaries split the batch into per-`__typename` buckets
