...
graph we call the _operation plan_. Whilst doing this, <grafast /> keeps track
of which fields returned which steps, and uses this information to form the
_output plan_. Finally the _operation plan_ is optimized and finalized and the
_output plan_ is finalized, then they are ready for execution.
...
<grafast /> then finalizes the plan by calling the `finalize` method on steps
