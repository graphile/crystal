...
errored or were inhibited via [`inhibitOnNull`](./inhibitOnNull.mdx) or
[`inhibitIf`](./inhibitIf.mdx)) into ordinary (unflagged) data. `trap` wraps a
step and accepts an `acceptFlags` argument
...
`trap` is frequently paired with [`inhibitOnNull`](./inhibitOnNull.mdx),
[`inhibitIf`](./inhibitIf.mdx), or [`assertNotNull`](./assertNotNull.mdx) to
recover a usable value after guarding an upstream dependency. It also works
with side-effect steps: trapping an error allows you to continue a plan that
would otherwise halt once the error was raised.
