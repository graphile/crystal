...
- [loadOne][]: like a more powerful DataLoader.load &mdash; batch loading of single values
- [loadMany][]: like a more powerful DataLoader.load for one-to-many lookups &mdash; batch loading of lists of values per identifier
...
- [remapKeys][]: returns a step that results in an object made up of the given renamed
  keys and values from another object step (useful during optimization when a
  step inlines its work into an ancestor and wishes to replace itself with
...
- [inhibitOnNull][]: inhibit dependent work when the runtime value is null-ish
- [inhibitOnEmpty][]: inhibit dependent work when the runtime value is "empty"
- [inhibitIf][]: inhibit dependent work when a condition yields `true`
