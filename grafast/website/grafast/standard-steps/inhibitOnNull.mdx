...
With the current implementation, if all indices are inhibited then the step
does not execute. This is an implementation detail, not a guarantee on which
security or correctness may depend.
