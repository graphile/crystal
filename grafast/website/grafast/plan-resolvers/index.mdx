...
type that owns the field). For the root selection set, the parent object is
either represents the GraphQL `rootValue` or the subscription event. For all
...
The second argument, the "field arguments" (`fieldArgs`), is an object giving
