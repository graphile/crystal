...
The first argument, the "source step," is a step that represents the data from
the source that the field is being resolved against &mdash; i.e. the type that
owns the field &mdash; which will have been returned directly or indirectly from
a parent field (except in the case of the root selection set).

When the parent field is of an object type, the "source step" is simply the step
that the parent field resolved to. When the parent field has a list or abstract
type, the "source step" will be the resolved step that represents an entry from
the list, or the concrete object type for abstract types.
...
[FieldArgs](#fieldargs) section; but the main point is that you can retrieve a
step representing the value of an argument either using the `.getRaw(argName)`
method, or by extracting the `.$argName` property (dollar prefix preventing
clash with method names, and also indicating this is a step).
...
In a pure <Grafast /> schema (no traditional resolvers), if a field doesn't have
a plan resolver, a default plan resolver will be used that uses
[`get()`](../standard-steps/get.md) to retrieve the property of the parent
...
In "resolver emulation mode", the traditional `defaultFieldResolver` will be
emulated instead.
...
until a field with a plan is met; this impacts the behavior of the default plan
resolver.
...
Object types in <Grafast /> can indicate that they must be represented by a
