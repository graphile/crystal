...
abstract types must resolve to a concrete object type.

Gra*fast* supports both of these forms of GraphQL polymorphism, both through
traditional GraphQL.js resolvers and through plan resolvers. Resolvers work the
same (basically) as they do in GraphQL.js so we won't dig into them here, but
let's look into how Gra*fast* supports polymorphism via plan resolvers (which
...
resolver and the abstract type's `planType` method.

### Specifier and planType
...
call this "type-to-fetch", and for it the specifier is this string we mentioned:
`User:1`.
...
If you use `makeGrafastSchema()` then you can specify `__planType` on the
interface type:
...
an `info` object you can find out more about in "Advanced" below. The function
must return a `PolymorphicTypePlanner` object, which is an object with two
entries:

- `$__typename` (required): a step representing the name of the resolved
  concrete object object type.
...
```ts
function planType($specifier: Step<number>): PolymorphicTypePlanner {
...
```ts
function planType($specifier: Step<string>): PolymorphicTypePlanner {
...
We minimize this cost by fanning in before fanning out (see the below infobox), but
there's space for more optimization: at some point, Gra*fast* might add support for on-demand polymorphic planning.

With on-demand polymorphic planning, each "polymorphic branch" of the plan will only be planned
...
never needing to be planned at all! If this is something your deployment of
Gra*fast* needs, please get in touch!
...
"fanning out" in polymorphism to avoid expontential branching and that choice
has had a significant impact on the shape of the APIs we make available.

If planned naively, nested polymorphic positions could lead to significant
branching. Imagine a polymorphic type such as `Item` that can represent `P`
different concrete object types, and imagine that `Item` contains a relationship
to another `Item` (e.g. `parentItem: Item`). Now imagine a GraphQL request that
queries this `D` levels deep:

<!-- prettier-ignore -->
```graphql
query ItemAndAncestors($id: ID!) {
  item(id: $id) {
    ...ItemDetails
    parentItem {
      ...ItemDetails
...
fragment ItemDetails on Item {
  ...
  ... on BarItem { bar }
  ... on BazItem { baz }
  # And so on, P types wide
}
```

If we were to plan this naively, recursing through each different type at each
level, there would be up to `P^D` different branches - the complexity would
...
before it "fans out" again, so for ten types (`P=10`) and five levels (`D=10`)
...
already represents the relevant data and you already needed for some reason.
However, if this field is queried through multiple polymorphic paths Gra*fast*
will want to perform "fan in" to prevent excessive branching. To do so it will
...
For consistency, we also call `toSpecifier($step)` (if present) when there's
...
step then we also make available the original step via the `info` argument
...
Though we limit the impact of polymorphism, it's still quite expensive compared
to regular fields (you can think of it as more akin to the number of fields that
there are types that polymorphic position represents). As such, to protect your
servers from malicious queries we recommend that you implement trusted documents
if you can, and if not that you apply a polymorphic depth limit to avoid
polymorphisms branching becoming an attack vector for your schema. We also make
available planning timeouts, but these should be your fallback defence rather
than your primary defence.
