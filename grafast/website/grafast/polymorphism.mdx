...
supports polymorphism via plans (which enables greater efficiency!).
...
- `planForType(t)` (optional): a function that is called for each possible
  concrete object type, `t`, and must return the step to use for that type. If
...
inside the `planForType` method, and less preliminary logic:
  ...
  return {
    ...
    planForType(t) {
      ...
      switch (t.name) {
        ...
        case "Organization": {
          const $id = get(parsed, "id");
          return organizations.get({ id: $id });
        }
        default: {
...
If we were to plan this naively, walking each different type separately at each
...
before it "fans out" again, so for ten types (`P=10`) and five levels (`D=5`)
instead of having up to 100,000 (`P^D`) branches; Grafast scales linearly with
depth meaning there's up to 50 (`P*D`) branches.

### Advanced

TODO: write about `toSpecifier`, `info: PlanTypeInfo`, and `info.$original`.

## Caveats

We recommend that you implement trusted documents if you can, and if not that
you apply a polymorphic depth limit to avoid polymorphisms branching becoming an
attack vector for your schema.
...
At some point, Gra*fast* might add support for on-demand polymorphic planning.
