...
and plans are reused for similar future requests; the cost of an additional
...
If you have a fixed number of dependencies it's common to destructure them at the
...
class MyStep extends Step {
  ...
  execute(details) {
    ...
    return details.indexMap((batchIndex) => {
      // Batch dependencies may have a different value at each index.
...
`$select2.deduplicatedWith($select1)`. This would give `$select2` a chance to
...
concerns and should not attempt to communicate with its ancestors or descendants
...
abstract types (unions and interfaces) that you've implemented. We strongly
...
Helper steps like `sideEffect()` set this flag for you; but you can set it
directly on any step:

```ts
const $step = doSomething();

const $log = lambda(null, () => console.log("hello world"))
// Ensure $log will run and not be tree shaken away
$log.hasSideEffects = true;
...
- The `execute` method must NEVER return a promise, async iterator, or similar
- The values within the list returned from `execute` must NEVER include promises,
  async iterators, or `FlaggedValue` objects such as errors
