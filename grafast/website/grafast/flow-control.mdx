...
load Z", but Gra*fast* does not exist to perform such logic - it is not a Turing
complete programming language, instead it is a planning system that defines and
optimizes the flow of data. All procedural and business logic belong inside the
steps themselves, and a field plan must return exactly one step that represents
data of the expected return type - so the decision as to which specific `Post` a
field resolver should return (whether the English translation or the German one,
for example) belongs inside the relevant single step that loads the `Post`, or
maybe one of its dependencies.
