...
function, though it's also really important to memoize the parsing of documents
to ensure that the same operation plan can be used again in future requests.

You can use any GraphQL server that allows you to substitute the `execute`
method with Grafast's; we have our native [Grafserv][] server, or if you don't
need the absolute best possible performance you can use any of the GraphQL
servers that Envelop supports with our `useGrafast` envelop plugin.
...
`useParserCache()` to get an optimized execution pipeline very easily, here's
part of an example:
