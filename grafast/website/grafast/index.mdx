...
get the data; since the Grafast plan resolver cannot loop through the data (as
it does not exist yet), it uses an `each` helper to detail which steps to take
for each item available later.
