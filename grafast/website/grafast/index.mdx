...
compared to traditional GraphQL execution.
...
When a step executes, its `executeV2` method is passed the **execution
...
`executeV2` is our second signature for step execution; the old `execute`
method is still supported but is less efficient.

:::

The `executeV2` method must return a list (or a promise to a list) of length
...
Each entry, `dep`, in the values tuple may be either a batched
entry (representing `count` entries) or a unary entry (representing exactly one
value); either way, the Nth value can be retrieved via `dep.at(N)`.
...
&ZeroWidthSpace;<grafast /> will automatically determine which other steps are also unary
steps.

<!-- For any execution value `dep` in the values tuple, `dep.at(i)` is equivalent to
`dep.isBatch ? dep.entries[i] : dep.value`.

 Since we know that the value of a unary step will be the same
for every entry in the batch no matter the batch size, so we can access it via
`dep.value` directly without having to look it up by index. -->
...
export class RecordsByColumnStep extends ExecutableStep {
  ...
  async executeV2({ count, values }) {
    ...
    const placeholderValues = Array.from({ length: count }, (_, i) =>
      // The value from `$columnValue` for this index `i` in the batch
      columnValueDep.at(i),
    );
    ...
    const rows = await executeSQL(sql, placeholderValues);

    // Figure out which rows relate to which batched inputs:
    return Array.from({ length: count }, (_, i) =>
      rows.filter((row) => row[this.columnName] === columnValueDep.at(i)),
    );
  }
}
...
The `executeV2` method is just JavaScript; it can
