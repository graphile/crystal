...
[^1]:
  You can opt out of batch processing on a per-step basis (like `lambda()` and
  `sideEffect()` do), but Gra*fast*'s model is built around batching. Unbatched
  execution is an optimization for certain shapes of logic (typically
...
step](/grafast/step-library/standard-steps/each), detailing what to do with each
