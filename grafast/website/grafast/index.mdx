...
Where in a traditional schema you would add a "resolver," in a <grafast />
schema you would instead add a "plan resolver." These plan resolvers are
...
are the building blocks of an execution plan.
...
In <grafast />, we use "plan resolvers" instead, which might look something
like:
...
&ZeroWidthSpace;<grafast /> would build an operation plan for the operation, which for the above
query is:
...
this most optimal form, made possible by Grafast's plan lifecycle.
