...
and a <grafast /> schema can query any data source or business logic that
Node.js can query.
...
Where in a traditional GraphQL schema you would add a resolver, in a <grafast
/> schema you would instead add a
[**plan&nbsp;resolver**](/grafast/plan-resolvers). These plan resolvers are
generally small functions, like regular resolvers should be, but instead of
being called many times and dealing with concrete runtime values, they are
called only once at planning time and they build and manipulate **steps** which
are the building blocks of an [**execution
plan**](/grafast/operation-plan#execution-plan).
...
but instead to outline the steps required to get the data. As the `friends`
Gra*fast* plan resolver cannot explicitly loop through the data (it hasn't
been fetched yet!), it uses an
[`each` step](/grafast/step-library/standard-steps/each) to detail which steps
to take for each item made available later.
...
Creating an operation plan should only happen once for each new GraphQL
operation that's seen, when the same operation is seen again its existing plan
...
However, if you leverage them, your performance can go from "good" to
