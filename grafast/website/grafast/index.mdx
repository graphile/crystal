...
Before Gra*fast*, there was one main GraphQL execution engine available in
...
resolvers. Other projects built on this, for example [graphql-jit][], which
aimed to reduce some of the graphql-js overheads via precompilation, but
ultimately still relied on the "resolvers" approach.
...
opaque resolvers[^1], favoring instead a planning strategy that unlocks the
potential for significant optimizations not previously achievable without a
herculean effort.

[^1]:
    Grafast still supports resolvers, but it's recommended that users only use
    them as a way of supporting legacy schemas - new functionality should be
    added via plan resolvers.

## Plans

Grafast plan resolvers are generally small functions, like regular resolvers
should be, but instead of being called many times and dealing with concrete
runtime values, they are called only once at planning time and they deal with
"steps" which are the building blocks of an execution plan.
...
resolvers are not asynchronous and do not return or deal with promises. Their
job isn't to get the data to use, but instead to outline the steps required to
get the data; hence the Grafast plan resolver cannot loop through the data
(since the data does not exist yet), it uses an `each` helper to detail which
steps to take for each item available later.
...
All plan lifecycle methods are optional, and you can get good performance with
Grafast without using any of them. However, if you leverage them, your
performance can go from "good" to amazing. One of the great things about
Grafast's design is that you don't need to build these optimizations from the
start, you can implement them at a later stage and your schema will get faster
even though the plan resolvers themselves don't have to change!

As a very approximate overview:

- every time a field is planned, we **deduplicate** its steps vs existing steps
...
new step, thereby reducing the complexity of the plan.

### Optimize

**Optimize** lets a step "talk" to its ancestors, typically to tell them about
data that they will need. This should not change the observed behavior of the
...
query; the explicit listing of the columns saved around 15% of the execution
time, even though there was only roughly another 160 bytes per row in these
examples. Were these tables much wider with larger columns (perhaps things like
JSON objects, or avatar blobs) the savings would have been much more pronounced.

In more advanced plans (for example those made available through
[@dataplan/pg][], our Postgres step collection), optimize can go much further
and even do things like inlining a query into its parent and replacing itself
with a simple "mapper" function.
...
query once. A step that converts a tuple into an object might build an optimized
function to do so.
...
If you're not convinced, please do reach out to Benjie on the [Graphile Discord][]
with your queries, he'd love to make improvements to both this page, and Grafast itself!

If you are convinced, why not continue on to getting started...
