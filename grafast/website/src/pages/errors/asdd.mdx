...
:::tip[Guard against captured steps]

Add a lint or review check that flags closures capturing steps from an outer
scope, especially in `each()` callbacks. It prevents accidental use of steps
that have already been deleted.

If you create such a lint rule, please consider contributing it back to the
community!
