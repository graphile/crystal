...
export interface PgSelectOptions<TResource extends AnyPgResource> {
  /**
...
export interface AnyPgSelectStep extends PgSelectStep<any> {}
 ...
 */
export class PgSelectStep<TResource extends AnyPgResource>
    ...
    >
{
  ...
  private relationJoins: Map<
    GetPgResourceRelationConfigs<TResource>["name"],
    SQL
  >;
  ...
  // DO NOT REMOVE THE `any` here. It'll give massive hard to track type errors if you do.
  // I suspect it's due to circular type inference issues.
  private locker = new PgLocker<any>(this);
  ...
  public placeholder($step: PgTypedExecutableStep<AnyPgCodec>): SQL;
  public placeholder($step: ExecutableStep, codec: AnyPgCodec): SQL;
  public placeholder(
    $step: ExecutableStep | PgTypedExecutableStep<AnyPgCodec>,
    overrideCodec?: AnyPgCodec,
  ): SQL {
    if (this.locker.locked) {
...
${lateralText};`;
  ...
  deduplicate(peers: PgSelectStep<any>[]): PgSelectStep<TResource>[] {
    ...
    return peers.filter(($p): $p is PgSelectStep<TResource> => {
      ...
      const p = $p as PgSelectStep<AnyPgResource>;
  ...
  private mergeSelectsWith<TOtherStep extends PgSelectStep<AnyPgResource>>(
  ...
  private mergePlaceholdersInto<TOtherStep extends PgSelectStep<AnyPgResource>>(
    otherPlan: TOtherStep,
  ): void {
  ...
  optimize({ stream }: StepOptimizeOptions): ExecutableStep {
    ...
    if (
      ...
      let t: PgSelectStep<AnyPgResource> | null | undefined = undefined;
   ...
   */
  singleAsRecord<
    TSelectSinglePlanOptions extends PgSelectSinglePlanOptions<TResource>,
  >(options?: TSelectSinglePlanOptions): PgSelectSingleStep<TResource> {
    this.setUnique(true);
   ...
   * one record then you're potentially in for a Bad Time.
   */
  single<TSelectSinglePlanOptions extends PgSelectSinglePlanOptions<TResource>>(
    options?: TSelectSinglePlanOptions,
  ): [GetPgResourceAttributes<TResource>] extends [never]
  ...
  row<TSelectSinglePlanOptions extends PgSelectSinglePlanOptions<TResource>>(
    $row: ExecutableStep,
    options?: TSelectSinglePlanOptions,
  ) {
  ...
  ): TResource extends AnyScalarPgResource
    ? PgClassExpressionStep<PgResourceCodec<TResource>, TResource>
    : PgSelectSingleStep<TResource>;
  listItem(
    itemPlan: ExecutableStep,
  ):
 ...
 */
function ensureOrderIsUnique<
  TResource extends PgResource<any, any, AnyPgResourceUnique, any, any>,
>(step: PgSelectStep<TResource>) {
  // No need to order a unique record
  if (step.unique()) return;
  const unique = step.resource.uniques[0];
  if (unique !== undefined) {
    const ordersIsUnique = step.orderIsUnique();
    if (!ordersIsUnique) {
      unique.attributes.forEach((c) => {
        step.orderBy({
          fragment: sql`${step.alias}.${sql.identifier(c)}`,
...
export function pgSelect<TResource extends AnyPgResource>(
 ...
 */
export function pgSelectFromRecords<TResource extends AnyPgResource>(
  resource: TResource,
  records:
    | PgClassExpressionStep<
        PgCodec<any, never, any, any, GetPgResourceCodec<TResource>, any, any>,
        TResource
      >
    | ExecutableStep<any[]>,
): PgSelectStep<TResource> {
...
export function getFragmentAndCodecFromOrder(
  alias: SQL,
  order: PgOrderSpec,
  codecOrCodecs: AnyPgCodec | AnyPgCodec[],
): [fragment: SQL, codec: AnyPgCodec, isNullable?: boolean] {
