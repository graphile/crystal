...
export function makeScopedSQL<TThis extends { placeholder(value: any): SQL }>(
  that: TThis,
): <T>(cb: PgSQLCallbackOrDirect<T, TThis>) => T {
  function isThis(value: any): value is TThis {
    return value === that;
  }
  const sqlTransformer: Transformer<PgEmbeddable | TThis> = (sql, value) => {
    if (isThis(value)) {
      if (hasAlias(that)) {
        return that.alias;
  ...
  return <T>(cb: PgSQLCallbackOrDirect<T, TThis>) =>
    typeof cb === "function"
      ? sql.withTransformer(sqlTransformer, cb as PgSQLCallback<T, TThis>)
      : cb;
}

const runtimeSQLTransformer: Transformer<PgEmbeddable> = (sql, value) => {
  if (hasAlias(value)) {
    return value.alias;
  } else {
...
export function runtimeScopedSQL<T>(cb: PgSQLCallbackOrDirect<T, never>): T {
  return typeof cb === "function"
    ? sql.withTransformer(runtimeSQLTransformer, cb as PgSQLCallback<T, never>)
    : cb;
}
