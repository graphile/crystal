...
// TODO: is this _even_ suitable for tests? It's barely tested at all... it's just a copy of the above with a couple hacks applied.
/**
      ...
      try {
        const client: PgClient = {
                        ...
                        if (connection.parsedStatements[key]) {
                          pgClient
                            ...
                            .catch((e) => {
                              // eslint-disable-next-line no-console
                              console.error(
                                "Error releasing prepared query",
                                e,
                              );
                            });
                        }
          ...
          async commitTransaction() {
            txLevel--;
            if (txLevel === 0) {
              throw new Error("FORBIDDEN COMMIT");
            } else if (txLevel > 0) {
              await pgClient.query({ text: `release savepoint tx${txLevel}` });
            } else {
              catastrophicFailure = new Error(
