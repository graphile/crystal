...
const errorSuffix = \`\${RESET}\n\${BOLD_BLACK_ON_RED}[ CONTEXT ]\${RESET} \${RED}You are attempting to run pre-release software that is only available to Graphile sponsors.\${RESET}\n\nPlease refer to the \${website} documentation for the right code to use with this version '\${BOLD}\${pkg.version}\${RESET}'.\${RESET}\n\`;

const password = process.env.I_SPONSOR_GRAPHILE;
const commonErrorMessage = \`Please refer to the https://grafast.org documentation for the right code to use with this version \${pkg.version}.\`;
