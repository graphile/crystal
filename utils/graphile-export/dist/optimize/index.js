...
function patch(scope) {
    if (!scope[$$patched]) {
        ...
        scope._replaceWith = function (...args) {
...
patch(traverse_1.NodePath.prototype);
    ...
    (0, traverse_1.default)(ast, {
                ...
                if (node.declarations.length === 1) {
                    ...
                    if (name.type === "Identifier") {
                        ...
                        if (fn && fn.type === "FunctionExpression") {
                            const functionDecl = t.functionDeclaration(name, fn.params, fn.body, fn.generator, fn.async);
        ...
        CallExpression: {
            enter(path) {
                ...
                const getExpression = (functionBody) => {
...
function expressionIsAlwaysFalsy(test) {
    switch (test.type) {
        case "Identifier":
        ...
        case "BinaryExpression": {
            switch (test.operator) {
                case "!=": {
                    if (expressionIsNullOrUndefined(test.left) &&
...
function expressionIsAlwaysTruthy(test) {
    switch (test.type) {
        case "BooleanLiteral":
            return test.value;
        default:
...
function expressionIsNullOrUndefined(expr) {
    return (expr.type === "NullLiteral" ||
