...
exports.resolveResourceRefPath = void 0;
exports.tagToString = tagToString;
exports.parseSmartTagsOptsString = parseSmartTagsOptsString;
exports.parseDatabaseIdentifier = parseDatabaseIdentifier;
exports.parseDatabaseIdentifiers = parseDatabaseIdentifiers;
...
function parseSmartTagsOptsString(optsString, leading = 0) {
    ...
    function validateParameterName(name) {
        ...
        if (!SAFE_PARAMETER_NAME.test(name)) {
            throw new Error(`Invalid parameter name '${name}' - please carefully check your syntax, especially the placement of spaces and quotes. (Opts string: ${JSON.stringify(optsString)})`);
        }
        return name;
    }
    function doneOne() {
        ...
        if (inQuotes) {
            ...
            if (char === CLOSE_PARENS[stack[stack.length - 1]]) {
                ...
                if (parenIdx >= 0) {
                    ...
                    throw new Error(`Found mismatched '${char}' at position ${i} (closing parenthesis; expecting '${CLOSE_PARENS[stack[stack.length - 1]]}'); if this is deliberate, be sure to escape your parameter value with quotes. (Opt string = ${JSON.stringify(optsString)})`);
                }
            }
        }
        else {
                    ...
                    if (IGNORED.test(char)) {
                        ...
                        if (char === '"') {
                            ...
                            if (openIdx >= 0) {
                                ...
                                throw new Error(`Argument value started with '${char}' which is a close parenthesis character; if this is intentional, please escape the argument value with double quotes.`);
                            }
                        }
                    }
                    break;
                ...
                case MODE.EXPECT_PARAM_NAME: {
                    if (IGNORED.test(char)) {
                        ...
                        if (openIdx >= 0) {
                            ...
                            throw new Error(`Param name started with '${char}' which is not permitted.`);
                        }
                    }
                    break;
                }
                    ...
                    if (IGNORED.test(char)) {
                        ...
                        if (char === '"') {
                            ...
                            if (openIdx >= 0) {
                                ...
                                throw new Error(`Parameter value started with '${char}' which is a close parenthesis character; if this is intentional, please escape the Parameter value with double quotes.`);
                            }
                        }
                    }
                    break;
                ...
                case MODE.PARAM_VALUE: {
                    if (IGNORED.test(char)) {
                        ...
                        if (parenIdx >= 0) {
                            ...
                            throw new Error(`Found unexpected '${char}' at position ${i} (closing parenthesis; but no parenthesis is open); if this is deliberate, be sure to escape your parameter value with quotes. (Opt string: ${JSON.stringify(optsString)})`);
                        }
                    }
                    break;
                }
...
function parseDatabaseIdentifier(identifier, expectedLength, fallbackNamespace = "public") {
    const identifiers = parseDatabaseIdentifiers(identifier, expectedLength, fallbackNamespace);
    if (identifiers.length === 1) {
        return identifiers[0];
    }
    else {
        throw new Error(`Was expecting '${identifier}' to contain exactly 1 identifier`);
    }
}
/**
 ...
 * `parseIdentifiers()`.
 */
function parseDatabaseIdentifiers(identifier, expectedLength, fallbackNamespace = "public") {
    const identifiers = [];
    let currentParts = [];
    let currentIdentifier = "";
    let state = "EXPECT_IDENTIFIER";
    function nextIdentifier() {
        if (currentIdentifier !== "") {
            throw new Error(`GraphileInternalError<05c47638-c2cd-4b47-bfc2-67ccbf861e2c>: bug in identifier parser - currentIdentifier should be blank at this point`);
        }
        if (currentParts.length > expectedLength ||
            currentParts.length < expectedLength - 1) {
            throw new Error(`Cannot parse database identifier '${identifier}' - it has the wrong number of parts`);
        }
        const parts = currentParts.length < expectedLength
            ? [fallbackNamespace, ...currentParts]
            : currentParts;
        ...
        switch (state) {
            ...
            case "QUOTED_IDENTIFIER": {
                if (c === '"') {
                    ...
                    throw new Error(`GraphileInternalError<72452ea1-715a-4a7b-93d5-49b5348a9c16>: parser for identifiers currently does not support backslashes`);
                }
                else {
                    currentIdentifier += c;
                }
            ...
            case "EXPECT_NEXT": {
                if (c === ".") {
                    ...
                    throw new Error(`Unexpected character '${c}' at position '${i}' in identifiers string '${identifier}'`);
                }
                break;
            }
        }
