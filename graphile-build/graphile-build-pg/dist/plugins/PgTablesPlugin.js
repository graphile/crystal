        ...
        helpers: {
            getResourceOptions(info, serviceName, pgClass) {
                ...
                resourceOptions = (async () => {
                    ...
                    const finalOptions = (0, graphile_build_1.EXPORTABLE_OBJECT_CLONE)(options);
            ...
            pgResourceUnique: {
                inferred: {
                    ...
                    callback(behavior, [resource, _unique]) {
                        return unloggedOrTempBehaviors(resource.extensions, behavior, resource);
                    },
                },
            },
        },
                    ...
                    build.recoverable(null, () => {
                        ...
                        // && build.behavior.pgCodecMatches(codec, "insert")
                            ...
                            build.registerInputObjectType(inputTypeName, {
                                ...
                                extensions: {
                                    grafast: {
                                        baked: grafast_1.createObjectAndApplyChildren,
                                    },
                                },
...
function unloggedOrTempBehaviors(extensions, behavior, resource) {
    const isUnloggedOrTemp = extensions?.pg?.persistence === "u" || extensions?.pg?.persistence === "t";
    return [
        ...(resource && !resource.parameters ? ["resource:select"] : []),
        behavior,
        ...(isUnloggedOrTemp
            ? [
                "-resource:select",
