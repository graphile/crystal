    ...
    inflection: {
        add: {
            ...
            _getBaseNameFromKeys(preset, detailedKeys) {
                ...
                if (preset.schema?.pgSimplifyMultikeyRelations) {
                    ...
                    const baseNames = attributeNames.map((attributeName) => this._getBaseName(attributeName));
                    // Check none are null
        ...
        replace: {
            ...
            deletedNodeId(_prev, preset, { resource }) {
                return this.camelCase(`deleted-${this.singularize(this.tableType(resource.codec))}-${this.nodeIdFieldName()}`);
            },
            ...
            allRowsConnection(previous, options, resource) {
                ...
                return overrideListSuffix(listSuffix, () => {
                    if (options.schema?.pgSimplifyAllRows) {
                        return this.connectionField(this.camelCase(`${this.distinctPluralize(this._singularizedResourceName(resource))}`));
                    }
                    else {
            ...
            allRowsList(previous, options, resource) {
                ...
                return overrideListSuffix(listSuffix, () => {
                    if (options.schema?.pgSimplifyAllRows) {
                        return this.listField(this.camelCase(this.distinctPluralize(this._singularizedResourceName(resource))));
                    }
                    else {
                        return previous(resource);
                    }
            ...
            allInterfaceModeUnionRowsConnection(previous, options, codec) {
                ...
                return overrideListSuffix(listSuffix, () => {
                    if (options.schema?.pgSimplifyAllRows) {
                        return this.connectionField(this.camelCase(`${this.distinctPluralize(this._singularizedCodecName(codec))}`));
                    }
                    else {
            ...
            allInterfaceModeUnionRowsList(previous, options, codec) {
                ...
                return overrideListSuffix(listSuffix, () => {
                    if (options.schema?.pgSimplifyAllRows) {
                        return this.listField(this.camelCase(this.distinctPluralize(this._singularizedCodecName(codec))));
                    }
                    else {
                        return previous(codec);
                    }
            ...
            singleRelation(previous, _options, details) {
                ...
                if (foreignPk &&
                    ...
                    return this.camelCase(`${this._singularizedCodecName(relation.remoteResource.codec)}`);
                }
            ...
            singleRelationBackwards(previous, _options, details) {
                ...
                const detailedKeys = relation.remoteAttributes.map((attributeName) => ({
                ...
                if (baseName) {
                    ...
                    if (oppositeBaseName) {
                        return this.camelCase(`${oppositeBaseName}-${this._singularizedCodecName(relation.remoteResource.codec)}`);
                    }
                    if (this._baseNameMatches(baseName, codec.name)) {
                        return this.camelCase(`${this._singularizedCodecName(relation.remoteResource.codec)}`);
                    }
                }
                const possibleResources = Object.values(registry.pgResources).filter((resource) => resource.codec === codec && !resource.parameters);
                if (possibleResources.length > 1) {
                    throw new Error(`singleRelationBackwards inflector check failed: multiple table-like resources for codec '${codec.name}', so we cannot determine the primary key - please override this inflector for this table.`);
                }
            ...
            _manyRelation(previous, _options, details) {
                ...
                if (baseName) {
                    const oppositeBaseName = this._getOppositeBaseName(baseName);
                    if (oppositeBaseName) {
                        return this.camelCase(`${oppositeBaseName}-${this.distinctPluralize(this._singularizedCodecName(relation.remoteResource.codec))}`);
                    }
                    if (this._baseNameMatches(baseName, codec.name)) {
                        return this.camelCase(`${this.distinctPluralize(this._singularizedCodecName(relation.remoteResource.codec))}`);
                    }
                }
            ...
            updateByKeysField(previous, options, details) {
                ...
                if (options.schema?.pgShortPk && unique.isPrimary) {
                    return this.camelCase(`update-${this._singularizedCodecName(resource.codec)}`);
                }
                else {
                    return previous(details);
                }
            ...
            deleteByKeysField(previous, options, details) {
                ...
                if (options.schema?.pgShortPk && unique.isPrimary) {
                    // Primary key, shorten!
                    return this.camelCase(`delete-${this._singularizedCodecName(resource.codec)}`);
                }
                else {
                    return previous(details);
                }
            ...
            updateByKeysInputType(previous, options, details) {
                ...
                if (options.schema?.pgShortPk && unique.isPrimary) {
                    // Primary key, shorten!
                    return this.upperCamelCase(`update-${this._singularizedCodecName(resource.codec)}-input`);
                }
                else {
                    return previous(details);
                }
            ...
            deleteByKeysInputType(previous, options, details) {
                ...
                if (options.schema?.pgShortPk && unique.isPrimary) {
                    // Primary key, shorten!
                    return this.upperCamelCase(`delete-${this._singularizedCodecName(resource.codec)}-input`);
                }
                else {
                    return previous(details);
                }
            ...
            updateNodeField(previous, options, details) {
                if (options.schema?.pgShortPk) {
                    return this.camelCase(`update-${this._singularizedCodecName(details.resource.codec)}-by-${this.nodeIdFieldName()}`);
                }
                else {
                    return previous(details);
                }
            ...
            deleteNodeField(previous, options, details) {
                if (options.schema?.pgShortPk) {
                    return this.camelCase(`delete-${this._singularizedCodecName(details.resource.codec)}-by-${this.nodeIdFieldName()}`);
                }
                else {
                    return previous(details);
                }
            ...
            updateNodeInputType(previous, options, details) {
                if (options.schema?.pgShortPk) {
                    return this.upperCamelCase(`update-${this._singularizedCodecName(details.resource.codec)}-by-${this.nodeIdFieldName()}-input`);
                }
                else {
                    return previous(details);
                }
            ...
            deleteNodeInputType(previous, options, details) {
                if (options.schema?.pgShortPk) {
                    return this.upperCamelCase(`delete-${this._singularizedCodecName(details.resource.codec)}-by-${this.nodeIdFieldName()}-input`);
                }
                else {
                    return previous(details);
                }
