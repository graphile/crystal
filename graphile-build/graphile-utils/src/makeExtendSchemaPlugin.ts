      ...
      hooks: {
        ...
        init(_, build, _context) {
          ...
          if (typeDefs === undefined) {
            throw new Error(
              "The first argument to makeExtendSchemaPlugin must contain a `typeDefs` field.",
            );
          }
          ...
          const mergedTypeDefinitions = typeDefsArr.reduce(
            (definitions, typeDef) => {
              if (!(typeDef as any) || (typeDef as any).kind !== "Document") {
                throw new Error(
                  "The first argument to makeExtendSchemaPlugin must contain a `typeDefs` field generated by the `gql` helper, or be an array of the same.",
                );
              }
