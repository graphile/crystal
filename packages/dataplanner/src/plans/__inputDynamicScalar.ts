...
import type { __TrackedObjectPlan } from "./__trackedObject.js";

/**
 * Handles "leaves" (scalars)
 */
export class __InputDynamicScalarPlan<
  TLeaf = any,
> extends ExecutablePlan<TLeaf> {
  static $$export = {
    moduleName: "dataplanner",
    exportName: "__InputDynamicScalarPlan",
  };
  isSyncAndSafe = true;
      ...
      switch (inputValue.kind) {
        case Kind.VARIABLE: {
          ...
          const variableValuePlan =
            this.aether.trackedVariableValuesPlan.get(variableName);
  ...
  eval(): TLeaf {
    const variableValues = this.variableNames.map((variableName, i) =>
      (this.getDep(i) as __TrackedObjectPlan).eval(),
    );
  ...
  evalIs(expectedValue: undefined | null | 0): boolean {
    if (
      ...
      throw new Error(
        "__InputDynamicScalarPlan doesn't support evalIs on non-null, non-undefined, non-0 values",
      );
    }
  }
}
