...
const debug = debugFactory("grafast:enforce");

/**
 * Ensures that all `resolve` and `subscribe` field methods in `schema` are
 * ✨wrapped in crystals✨.
 */
export function grafastEnforce(schema: GraphQLSchema): GraphQLSchema {
      ...
      for (const fieldName in fields) {
        if (
          ...
          if (!resolve || !isCrystalWrapped(resolve)) {
            ...
            field.resolve = grafastResolver(resolve);
          }
          ...
          if (
            ...
            if (subscribe && !isCrystalWrapped(subscribe)) {
              ...
              field.subscribe = grafastSubscriber();
            }
          }
        }
      }
