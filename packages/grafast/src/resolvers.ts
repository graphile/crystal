 ...
 */
function grafastResolverOrSubscriber<
...
>(
  ...
  const crystalResolver: GraphQLFieldResolver<TSource, TContext, TArgs> =
    function (
      ...
      throw new Error(
        "TODO<b85a2e03-2cd2-4b31-8360-525ca0630e80>: In-GraphQL resolution is not currently supported, please use the 'execute' or 'grafastGraphql' functions from 'grafast'.",
      );
    };
 ...
 */
export function grafastResolver<
  TSource extends object | null | undefined,
  TContext extends object,
  TArgs = { [argName: string]: any },
>(
  ...
  return grafastResolverOrSubscriber(resolve, false);
}

/**
 * If you don't need to wrap a resolver, you can use this to save memory.
 */
export const crystalResolve = grafastResolver(undefined);

/**
 * Returns a `subscribe` subscription resolver function that's capable of the
 * Grafast algorithms.
 */
export function grafastSubscriber<
  TSource extends object | null | undefined,
  TContext extends object,
  TArgs = { [argName: string]: any },
>(): GraphQLFieldResolver<TSource, TContext, TArgs> {
  return grafastResolverOrSubscriber(undefined, true);
}
