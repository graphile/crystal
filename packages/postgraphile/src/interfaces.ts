...
declare global {
  namespace GraphileEngine {
    ...
    > extends PostGraphileCoreOptions {
      ...
      // back-off between attempts, starting at 100ms and increasing up to 30s delay
      // between retries.
      /* @middlewareOnly */
      ...
      // proxy that puts PostGraphile under a sub-path (or both!) then you must
      ...
      // [DEPRECATED] The audience to use when verifying the JWT token. Deprecated,
      ...
      // ONLY use this option if you require the v3 type names 'Json' and 'Uuid'
      // over 'JSON' and 'UUID'.
      ...
      // you're comfortable with the security repercussions of doing so.
      /* @middlewareOnly */
...
export interface CreateRequestHandlerOptions
  extends GraphileEngine.PostGraphileOptions {
...
> {
  ...
  withPostGraphileContextFromReqRes: <T>(
    ...
    res: Response,
    moreOptions: any,
    fn: (ctx: GraphileEngine.GraphileResolverContext) => Promise<T> | T,
  ) => Promise<T>;
