...
>(
  ...
  if (
    ...
    (options.extendedErrors || options.showErrorStack)
  ) {
    throw new Error(
      `You cannot combine 'handleErrors' with the other error options`,
    );
  }
  ...
  return {
    _emitter,
    getGraphQLSchema: () =>
      gqlSchema ? Promise.resolve(gqlSchema) : gqlSchemaPromise,
    options,
  };
    ...
    while (true) {
      try {
        if (options.watchPg) {
          ...
          gqlSchema = await createPostGraphileSchema(
            pgPool,
            pgSchemas,
            options,
          );
