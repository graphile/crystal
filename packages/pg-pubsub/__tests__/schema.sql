drop schema if exists pubsub_test cascade;

create schema pubsub_test;
set search_path to pubsub_test, public;
...
create function odd_foos() returns setof foo as $$
  select * from pubsub_test.foo where id % 2 = 1;
$$ language sql stable;

create function odd_foos_array() returns foo[] as $$
  select array_agg(foo.*) from pubsub_test.foo where id % 2 = 1;
$$ language sql stable;

create function foo_one() returns foo as $$
  select * from pubsub_test.foo where id = 1;
$$ language sql stable;
